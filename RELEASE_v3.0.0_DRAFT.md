# XM10 Extension Module SDK v3.0.0 Release

> **릴리즈 날짜**: 2026-04-02 | **대상 HW**: Rev1.1 / Rev2.0 | **KIT H10 FW**: v2.3.0+
...
- **예제 42개** — 입문부터 고급 제어(Impedance, CPG, ILC, MRAC, DOB 등)까지 완전한 학습 경로
- **PhAI Studio 연동** 강화 — Total Data Packet 자동 전송 + User Custom 채널
- **정적 라이브러리 Release 빌드** — `-O2` 최적화로 더 작고 빠른 바이너리
...
#### 정적 라이브러리 Release 최적화
- libXM_Lib.a 빌드: `-Og -g3` (Debug) → **`-O2 -g0` (Release)**
- Rev1.1: 598KB (59 obj) | Rev2.0: 525KB (66 obj)
- `boot_fw_info.c`는 SDK에서 직접 컴파일 (링커 GC 회피, 사용자 버전 자동 반영)

#### Post-Build 자동화 파이프라인
- STM32CubeIDE 빌드 시 4단계 Post-Build 스크립트 자동 실행
- `size_report.py` → `version_generator.py` → `patch_fw_info.py` → `fw_packager.py`
- 최종 출력: `XM10_X_X_X_X.bin` (PhAI Studio FTP 업로드용 패키징 바이너리)

#### 예제 대규모 확장 (13개 → 42개)
- **물리 기반 AI 제어 시리즈** (Ex.20~33): Impedance, Gravity Comp, CPG, Gait Phase Adaptive, Virtual Constraint, Stance Stiffness, ILC, MRAC, Admittance, Bilateral, FF/FB Hybrid, Friction DOB, GRF Gait Intent, Kinesthetic Teaching
...
- `.cproject` SDK 설정: `-lXM_Lib` + `-L XM_FW/` 링커 설정 추가
...
- SDK에서 불필요 빌드 스크립트 제거: `cproject_to_cmake.py`, `patch_cubemx_overrides.py`
...
| 컴포넌트 | 최소 버전 | 권장 버전 | 비고 |
|----------|----------|----------|------|
| **XM10 FW (이 SDK)** | v3.0.0 | v3.0.0 | Rev1.1/Rev2.0 별도 libXM_Lib.a |
