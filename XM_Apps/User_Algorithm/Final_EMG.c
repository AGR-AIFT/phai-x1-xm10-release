...
static void Active_Loop(void)
{
    ...
    if (control_ON == 1U) {
        XM_SetAssistTorqueRH(rh_torque*scale[0]);
        XM_SetAssistTorqueLH(lh_torque*scale[1]);
    } else {
        XM_SetAssistTorqueRH(0.0f);
        XM_SetAssistTorqueLH(0.0f);
    }
...
static void _SelectTorquePair(float *rh_torque, float *lh_torque)
{
    if (torque_input_pair == 1U) {
        *rh_torque = s_envelope_v[CH_PF5];
        *lh_torque = s_envelope_v[CH_PF6];
    } else {
//        *rh_torque = s_torque_nm[CH_PF3];
//        *lh_torque = s_torque_nm[CH_PF4];
    	*rh_torque = s_envelope_v[CH_PF3];
    	*lh_torque = s_envelope_v[CH_PF4];
    }
}
