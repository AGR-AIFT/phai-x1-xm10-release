 ...
 * 
 * @note
 * - conversion_mode 필드는 H7/G4 공통이지만, 내부 처리 방식이 다릅니다:
 *   - H7: ConversionDataManagement 필드 직접 설정
 *   - G4: DMAContinuousRequests 필드로 변환
 */

/**
 * @brief ADC 수동 초기화 설정 구조체
 * @details 
 * - 모든 필드는 HAL 매크로 사용 (예: ADC_RESOLUTION_16B)
 * - 범용 API이므로 특정 사용 사례에 종속되지 않음
 * - ✅ H7/G4 공용: conversion_mode 필드는 H7/G4 모두 지원
 * - ✅ DMA 정보는 System Layer에서 주입 (IOIF는 고정 맵핑 안함!)
 */
typedef struct {
    uint32_t resolution;           /**< ADC_RESOLUTION_12B, _14B, _16B */
    ...
    uint32_t conversion_mode;      /**< [H7] ADC_CONVERSIONDATA_DMA_ONESHOT/CIRCULAR, [G4] 내부 변환 */
    bool oversampling_enable;      /**< Oversampling 활성화 여부 (16-bit ADC용) */
