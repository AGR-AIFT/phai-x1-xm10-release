...
// --- 제어 루프 타이밍 ---
#define CONTROL_DT          0.002f  // 제어 주기 (2ms = 500Hz)
...
/**
 * @brief ACTIVE 루프 — PD 제어기 실행 (2ms 주기)
 * @details
 * 매 루프마다 다음을 수행합니다:
 * 1. 버튼 입력 처리 (목표 각도 변경)
 * 2. PD 제어 연산
...
static void _UpdateUsbDebug(float current_angle, float error)
{
    ...
    if (now - s_usb_debug_timer >= USB_DEBUG_PERIOD_MS) {
        ...
        // 정수부와 소수부를 분리하여 출력
        int tgt_int = (int)s_target_angle;
        int tgt_dec = (int)(fabsf(s_target_angle - (float)tgt_int) * 10.0f);
        int cur_int = (int)current_angle;
        int cur_dec = (int)(fabsf(current_angle - (float)cur_int) * 10.0f);
        int err_int = (int)error;
        int err_dec = (int)(fabsf(error - (float)err_int) * 10.0f);
        int tau_int = (int)s_torque_cmd;
        int tau_dec = (int)(fabsf(s_torque_cmd - (float)tau_int) * 100.0f);

        char buf[80];
        snprintf(buf, sizeof(buf),
                 "PD | Tgt:%d.%d Cur:%d.%d Err:%d.%d Tau:%d.%02d\r\n",
                 tgt_int, tgt_dec, cur_int, cur_dec,
                 err_int, err_dec, tau_int, tau_dec);
