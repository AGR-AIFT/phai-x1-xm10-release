 ...
 *     2ms 루프에서는 0/1/2 중 하나의 값만 관측됩니다.
...
/** 루프 오버런 임계값 (ms) — XM10 User_Loop 주기는 2ms */
#define LOOP_PERIOD_MS          (2U)
...
void User_Loop(void)
{
    /*
     * [실행 시간 측정 전략]
     * XM_GetTick()은 ms 해상도이므로 2ms 루프에서는 정밀도가 제한됩니다.
     * 대부분의 루프에서 0ms로 측정되며, 가끔 1~2ms가 관측됩니다.
 ...
 * 하지만 오버런(>2ms) 감지는 시스템 이상 징후를 포착하는 데 유용합니다.
 */
static void _MeasureLoopTime(uint32_t elapsed_ms)
{
    s_loop_count++;
