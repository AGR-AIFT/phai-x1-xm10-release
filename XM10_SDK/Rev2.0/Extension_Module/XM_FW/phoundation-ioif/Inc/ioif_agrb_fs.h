...
/**
 * @brief [신규] 남은 용량과 전체 용량을 MB 단위로 가져옵니다.
 * @details f_getfree() 1회 호출로 free+total 동시 반환.
 *          기존 get_free_space_mb()와 동일한 f_getfree 로직을 사용하되 total도 함께 반환합니다.
 * @param[out] free_mb  남은 용량 (MB). NULL 전달 시 무시.
 * @param[out] total_mb 전체 용량 (MB). NULL 전달 시 무시.
 */
AGRBFileSystemStatusDef ioif_filesystem_get_space_mb(uint32_t* free_mb, uint32_t* total_mb);
...
/**
 * @brief 파일 R/W 포인터를 지정 위치로 이동합니다 (Pre-allocation용).
 * @details f_lseek()을 Mutex로 보호하여 호출합니다.
 *          offset이 파일 크기를 초과하면 FAT 클러스터를 확장합니다 (사전할당).
 * @param[in]  id      파일 핸들 ID
 * @param[in]  offset  파일 시작점으로부터의 바이트 오프셋
 * @param[out] fresult (선택) FatFS 에러 코드
 */
AGRBFileSystemStatusDef ioif_filesystem_seek(IOIF_FILEx_t id, uint32_t offset, FRESULT* fresult);

/**
 * @brief 파일을 현재 R/W 포인터 위치에서 잘라냅니다 (Pre-allocation 정리용).
 * @details f_truncate()를 Mutex로 보호하여 호출합니다.
 *          사전할당 후 정상 종료 시 실제 데이터 크기로 축소합니다.
 * @param[in]  id      파일 핸들 ID
 * @param[out] fresult (선택) FatFS 에러 코드
 */
AGRBFileSystemStatusDef ioif_filesystem_truncate(IOIF_FILEx_t id, FRESULT* fresult);
