...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  /* [2026-08-19 P1] Boot FTP 스캔(COBS decode+CRC16+Flash read+CDC TX)을 ISR 에서
   * 제거 — CdcDopRouter_Process()(UserTask 1kHz, StreamBuffer 소비자)로 이동.
   * ISR 은 raw bytes 전달만 수행한다 (Rule 13 ISR 최소화). */
...
uint8_t CDC_Transmit_FS(uint8_t* Buf, uint16_t Len)
{
  ...
  if (hcdc == NULL) {
    /* [2026-08-19 P1] USB role 전환 deinit 창(USBD_DeInit 이 pClassData=NULL 확정
     * 후 ~12ms) 동안 UserTask(prio53) producer 가 진입하면 NULL deref → HardFault
     * 였다. 최후 방어선 — 호출자들은 !=USBD_OK 를 이미 실패 처리함. */
    return USBD_FAIL;
  }
  if (hcdc->TxState != 0){
    return USBD_BUSY;
...
/**
 * @brief AGR_Boot_App TX wrapper — FTP 응답 송신 어댑터.
 * @note  [2026-08-19 P1] Called from CdcDopRouter_Process() task context (UserTask
 *        1kHz) via Boot_FTP_CheckForEnterBL() — ISR 스캔 defer 이후.
 *        구현도 CDC_Transmit_FS 직접 호출 → CdcStream_Send(SPSC 링) 경유로 전환:
 *        UserTask 에서의 직접 호출은 USB ISR 연쇄 전송(_TryTransmit)과 TxState
 *        check-and-set 이 경합해 EP 이중 arm 위험이 있었다. FTP 가 UserTask(=링의
 *        유일 producer)에서 실행되므로 링 경유가 단일 TX owner 원칙에 정합.
 *        (구 deferred-buf 재전송 경로는 휴면화 — CDC_TransmitCplt_FS 의 잔여
 *        로직은 무해하게 유지. 링 만석 시 응답 유실 가능하나 GUI 의 QUERY_INFO
 *        는 폴링형이라 재시도로 복구됨.)
 */
static int32_t _Boot_FTP_CdcTxWrapper(const uint8_t* data, uint32_t len)
{
    /* Suspend streaming — 응답이 센서 스트림에 묻히지 않게 신규 적재 중단.
     * (링에 이미 있는 데이터는 _TryTransmit 이 계속 내보냄 — 응답은 그 뒤에 실림) */
