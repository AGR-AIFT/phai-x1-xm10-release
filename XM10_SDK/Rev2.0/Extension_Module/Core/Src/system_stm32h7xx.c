...
void SystemInit (void)
{
 ...
 __IO uint32_t tmpreg;
  ...
  /* AGR_BOOT V2: Clean hardware state for BL+App coexistence.
   * Why: During SWD debug, BL at 0x08000000 may partially execute before the
   *      debugger halts and redirects PC to App. This leaves peripherals, SysTick,
   *      and NVIC in BL's configured state → App init fails without a manual reset.
   *      Mirrors BL's JumpToApp() cleanup (Phases 2-6) so App always starts clean.
   * Impact: SWD debug works without manual reset. Normal BL→App boot unaffected
   *         (BL already cleans up before jump, so this is a harmless no-op). */
  ...
  RCC->CFGR = 0U;                                   /* Switch system clock to HSI */
  while ((RCC->CFGR & RCC_CFGR_SWS) != 0U) {}      /* Wait for switch complete */
  RCC->CR &= ~(RCC_CR_PLLON | RCC_CR_PLL2ON | RCC_CR_PLL3ON
              | RCC_CR_HSEON | RCC_CR_CSION);        /* Disable PLL, HSE, CSI */
