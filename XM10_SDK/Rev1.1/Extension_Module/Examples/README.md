...
42 개의 실습 예제입니다(Rev 1.1 SDK 기준 — 전체 카탈로그는 46 개). 각 폴더에 소스 코드 `.c` 와 그 예제만의 설명서 `README.md` 가 함께 들어있습니다.
...
**보드 리비전 호환성** — 전체 46 개 예제 중 42 개는 Rev 1.1 / Rev 2.0 양쪽 모두 빌드·실행됩니다. 나머지 4 개(Ex.40 EMG Proportional Assist, Ex.41 IMU Hub Dashboard, Ex.42 EMG Hub Biofeedback, Ex.43 External UART Ping-Pong)는 Rev 2.0 전용이라 **이 Rev 1.1 SDK 에는 포함되지 않았습니다**(그래서 42 개). 그 외 두 가지 주의 사항이 있습니다.
...
각 예제의 README 는 다섯 단계로 정리되어 있습니다: ① 목표 / ② 사전 지식 / ③ 핵심 코드 / ④ 실험과 변형 / ⑤ 다음 단계. 마지막에 "흔한 실수" 도 함께 있습니다.

막혔다면 Claude Code 에서 `"Ex.XX 막혔어"` 라고 말해보세요. `example-helper` 가 해당 예제의 흔한 실수 섹션을 찾아 알려드립니다.

---

## 큰 그림 — 5 단계 학습 흐름

기본기를 다진 다음, 예제는 다섯 단계의 흐름 + 외부 센서 허브 연동(Ex.37, 40~42)을 따라갑니다. (Ex.40~43 은 Rev 2.0 전용이라 이 SDK 에는 없습니다.) "로봇이 인간을 어떻게 이해하고, 함께 성장하는가" 라는 한 줄로 묶을 수 있습니다.

```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Foundation   기본 입출력과 도구
             (00~09, 18, 19 — 언제든 돌아와 참고)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Stage 1      투명한 로봇 만들기
...
이 순서에는 이유가 있습니다. 로봇이 투명하지 않으면 (Stage 1) 인간 의도 신호가 마찰·중력 노이즈에 묻혀 보이지 않습니다 (Stage 2). 의도를 못 읽으면 보조 타이밍이 빗나갑니다 (Stage 3). 그래서 한 단계씩 쌓아 올리는 것입니다.

처음부터 5 단계를 다 가야 하는 건 아닙니다. **임베디드 처음이면 Foundation 만 한 학기 충분**, **제어 알고리즘에 관심 있으면 Stage 1~3**, **AI 응용이 목표면 Stage 4~5** 로 갈라집니다. 추천 경로는 아래 표를 참고하세요.
...
목표는 착용자가 로봇의 무게·마찰을 못 느끼게 하는 것입니다. Hogan(1985) 의 고전적인 정의로 "투명할수록 로봇의 임피던스가 0 에 가깝다". 여기서 만들어지는 외란 추정값이 다음 단계의 의도 감지 신호로 이어집니다.
...
원시 센서 신호 (GRF, IMU, 관절각) 에서 사람이 다음에 하려는 동작을 추출합니다. Ronsse(2011) 의 리드믹 동기화, Gervasi(2020) 의 연속 보행 위상 추정 같은 고전 방법부터 시작해서, 여기서 뽑은 의도 신호가 Stage 3 의 보조 트리거가 됩니다.
...
읽어낸 의도에 실시간으로 동기화해 보조 토크를 만들어냅니다. Quinlivan(2017) 의 보행 위상 동기화, Collins(2015) 의 최적 강성 같은 방법들입니다. 이 보조 전략을 다음 단계에서 학습 알고리즘으로 자동 최적화합니다.
...
반복 경험을 통해 제어기가 자기 파라미터를 스스로 조정합니다. Emken(2007) 의 반복 학습 제어 (ILC), Slotine(1991) 의 적응 제어 같은 방법들입니다. Stage 5 에서 전문가 교시로 모은 데이터가 이 단계의 학습 입력으로 들어옵니다.
...
사람과 AI 가 제어권을 분담하고, 말로 표현할 수 없는 전문가의 몸 감각을 데이터로 포착합니다. Dragan(2013) 의 공유 자율성, Polanyi(1966) 의 암묵지 개념입니다. 전문가가 투명 로봇을 착용한 채 직접 움직여 만든 궤적 데이터가 다음 세대 AI 모델의 학습 입력이 됩니다.
...
**Ex.33 한 줄 요약** — 전문가가 투명 모드로 만든 로봇을 직접 착용하고 동작을 보여주면, 100 Hz 로 궤적이 기록됩니다. 그 데이터가 VLA(Vision-Language-Action) 같은 차세대 AI 모델의 학습 소스가 됩니다. 말로 못 가르치는 장인의 손맛을 데이터로 옮기는 것입니다.
...
| 목표 | 추천 경로 |
|------|----------|
| **입문** (임베디드 처음) | 00 → 01 → 04 → 07 → 09 → 11 |
| **중급** (임베디드 기초) | 02 → 05a~d → 08 → 12 → 14 |
| **고급** (제어 알고리즘) | 03 → 06 → 13 → 15 → 16 → 17 |
...
Hogan(1985) 의 정의대로 임피던스를 0 에 가깝게. Ex.21 의 공칭 모델 보상에서 시작해 Ex.31 의 외란 관측기 (DOB) 까지 가면 진짜 투명 모드가 완성됩니다.
...
**Ex.31 한 줄 요약** — DOB 의 Q-filter 로 잔류 외란을 추정합니다. 그 외란값이 사실상 "사람이 가하는 힘" 이라서, 다음 단계 (의도 감지) 의 핵심 신호가 됩니다.

---

### Stage 2: 의도 읽기 (Ex.16, 17, 22, 32)

원시 센서 신호가 어떻게 "인간 의도" 로 변환되는지를 다룹니다. Ex.32 는 발 접지 이벤트 기반으로 연속 보행 위상을 추정합니다.
...
**Ex.32 한 줄 요약** — 발 뒤꿈치 접지에서 위상을 리셋하고, `phase += dt / T` 로 연속 위상을 업데이트합니다. sin 프로파일에 맞춰 보조 토크를 내보내면 보행에 자연스럽게 동기화됩니다.

---

### Stage 3: 맞춤 보조 (Ex.12, 14, 23, 25, 28)

감지한 의도에 실시간으로 동기화해 최적 보조 토크를 만들어냅니다.
...
말로 옮길 수 없는 전문가의 몸 감각을 외골격 교시로 데이터화합니다. Polanyi(1966) 가 "암묵지" 라고 불렀던 그 영역입니다. 이 데이터가 차세대 VLA(Vision-Language-Action) 모델의 학습 입력이 됩니다.
...
| 예제 | 제목 | 난이도 | 핵심 개념 |
| :---: | :--- | :---: | :--- |
| [38](38_Periodic_Background_Task/) | 주기적 백그라운드 태스크 | 중급 | 저주기 보조 작업 분리, 제어 루프 지터 최소화 |
| [39](39_Task_Lifecycle/) | 태스크 생명주기 관리 | 고급 | 태스크 생성·일시정지·종료, RTOS 태스크 상태 머신 패턴 |

---

## Part 7: 외부 센서 허브 · 외부 장비 연동 (Ex.40~43 — 전부 Rev 2.0 전용, 이 SDK 미포함)
...
1. 원하는 예제 폴더의 `.c` 파일과 `README.md` 를 엽니다.
