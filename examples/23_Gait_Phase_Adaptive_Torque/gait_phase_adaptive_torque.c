 ...
 *
 * [⚠️ Body Data 전제조건 — 필수]
 * gaitCycle, isFootContact, forwardVelocity는 H10 CM 실시간 보행 분석(1kHz) 출력입니다.
 * XM_SendUserBodyData()로 신체 데이터를 H10에 전달하지 않으면:
 *   - gaitCycle 부정확 → 토크 프로파일 타이밍 오류
...
{
    /* ⚠️ Body Data 필수 설정
     * gaitCycle/footContact/forwardVelocity 정확도를 위해 반드시 호출
     * 미설정 시 이 예제는 정상 동작하지 않습니다 */
    _SetupBodyData();
...
static void Active_Loop(void)
{
    ...
    /* gaitCycle(0~100) → 정규화 위상 φ (0~1), 위상 오프셋 적용 */
    float phi_raw  = (float)XM.status.h10.gaitCycle / 100.0f;
...
static void _UpdateUsbDebug(void)
{
    ...
    float phi = (float)XM.status.h10.gaitCycle / 100.0f;
    char buf[96];
...
static void _UpdateStreamData(void)
{
    s_stream_data.gait_phase = (float)XM.status.h10.gaitCycle / 100.0f;
