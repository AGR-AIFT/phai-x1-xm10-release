...
실측 토크: τ_joint = Kt_motor × gear_ratio × i_meas
          = 0.085 [Nm/A] × 18.75 × i_meas  ≈ 1.594 × i_meas [Nm]
  ※ XM.status.h10.rightHipTorque 필드는 모터 전류(A)를 제공합니다.
     필드명에 "Torque"가 붙어있으나 실제 단위는 Ampere(A)입니다.
     (내부 변환: raw_int16 / 60 → 범위 −30~+30 A)
     감속비(18.75)와 모터 Kt(0.085 Nm/A)를 곱하여 관절 토크로 환산합니다.
...
## 구동기 스펙 요약 (PhAI X1 기준)

| 항목 | 값 | 비고 |
|------|-----|------|
| 감속비 | 18.75 (양쪽 동일) | |
| 모터 Kt | 0.085 Nm/A | 데이터시트 |
| 관절 등가 Kt | ≈ 1.594 Nm/A | `0.085 × 18.75` |
| 정격 토크 | 10 Nm | 예제 가이드: **8 Nm 이하** |
| 하드웨어 최대 토크 | 18.3 Nm | MD 전류 보호 14A 기준: ≈22 Nm |
| 정격 속도 (관절) | 50 rpm → **16.0 deg/s** | 연속 운전 권장 범위 |
| 무부하 최대 속도 (관절) | 85 rpm → 27.2 deg/s | |
| 절대 최대 속도 (관절) | 120 rpm → 38.4 deg/s | |
| 역구동성 | ≤ 0.3 Nm | 투명 모드 기준 잔류 저항 |
| MD 전류 보호 | 14A 초과 → 경고 후 전원 OFF | |
| MD 온도 보호 | 110°C 이상 휴식 안내 / 120°C 이상 전원 OFF | |

> **처음 사용 시 권장**: `MAX_TORQUE_NM = 3.0 Nm`에서 시작하여 착용자 피드백에 따라 점진적으로 증가.
...
// rightHipTorque는 모터 전류(A). 관절 토크 = Kt_motor × gear_ratio × i
float current_r_a = XM.status.h10.rightHipTorque;            // A
float tau_meas_r  = KT_JOINT_NM_PER_A * current_r_a;         // 1.594 Nm/A × A = Nm ✓
//  KT_JOINT_NM_PER_A = KT_MOTOR_NM_PER_A(0.085) × GEAR_RATIO(18.75) ≈ 1.594
```

**주의**: `XM.status.h10.rightHipTorque`에 직접 Nm 단위로 사용하면 1.594배 오차가 발생합니다.
...
| `GEAR_RATIO` | 18.75 | 감속비 (양쪽 동일) | HW 변경 시만 수정 |
| `KT_MOTOR_NM_PER_A` | 0.085 Nm/A | 모터 토크 상수 (데이터시트) | MD 모터 사양서 참조 |
| `KT_JOINT_NM_PER_A` | ≈ 1.594 Nm/A | 관절 등가 토크 상수 (자동 계산) | `KT_MOTOR_NM_PER_A × GEAR_RATIO` |
| `DOB_CUTOFF_HZ` | 5 Hz | Q-filter BW | 낮을수록 느리고 안정, 높을수록 빠르고 채터링 |
| `MAX_TORQUE_NM` | **8.0 Nm** | 안전 포화 (예제 보수적 설정) | 정격 10 Nm 대비 안전 마진 확보. 처음 사용 시 3~5 Nm 권장 |
...
> 관절 토크 추정: `τ_joint = Kt_motor × gear_ratio × rightHipTorque = 0.085 × 18.75 × i ≈ 1.594 × i [Nm]`.
> 직접 Nm으로 사용하면 1.594배 오차가 발생합니다.
