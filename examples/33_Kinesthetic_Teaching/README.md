...
구동기 사양 기반 속도 한계를 이해하고 교시 속도를 제한하세요.

```
[구동기 속도 스펙] — 감속비 18.75 적용 후 관절 기준
  정격 속도:    50 rpm  → 관절  16.0 deg/s  ← 연속 운전 권장 범위
  무부하 최대:  85 rpm  → 관절  27.2 deg/s
  절대 최대:   120 rpm  → 관절  38.4 deg/s

→ 10ms 재생 주기에서 허용 각도 변화:
    정격 기준: 16.0 × 0.01 = 0.160 deg/step (권장)
    무부하 기준: 27.2 × 0.01 = 0.272 deg/step (상한)
```

- **권장 교시 속도**: 관절 각속도 **16 deg/s 이내** (정격 속도 기준, 연속 운전 안전).
- 이를 초과하면 모터가 추적에 실패 → PD 토크 포화 → 위치 오차 누적.

**추가 안전 한계:**
- MD 전류 14A 초과 시 보호 동작 (경고 후 전원 OFF) → 토크 포화 8 Nm으로 억제
- MD 온도 110°C 이상 휴식 안내, 120°C 이상 전원 OFF → 고강도 재생 반복 시 주의

```c
// 안전 개선 예시 (현재 미적용) — 과도한 교시 속도 방어
#define MAX_DELTA_DEG_PER_STEP   0.16f   // 16 deg/s × 10ms (정격 속도 기준)
float delta = target - prev_target;
delta = CLAMP(delta, -MAX_DELTA_DEG_PER_STEP, MAX_DELTA_DEG_PER_STEP);
...
> **속도 제한**: 교시 동작의 관절 각속도를 **16 deg/s 이내**(정격 속도 기준)로 유지하세요. 과도하게 빠른 교시 동작은 재생 시 모터 추적 실패 및 토크 포화를 일으킵니다.

> **MD 보호 한계**: 전류 14A 초과 시 MD가 경고 후 전원을 차단합니다. 토크 상한(`REPLAY_TORQUE_MAX = 8.0 Nm`)이 이를 방어하지만, 외부 충격이나 급격한 동작 시 일시적으로 초과될 수 있습니다. MD 온도가 지속적으로 높아지면(110°C 이상) 휴식을 취하세요.
