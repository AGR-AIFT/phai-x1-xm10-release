# Ex.33 — Kinesthetic Teaching (Stage 5 — 전문가 시연 → 캡처 → 재생)

> 🎯 **학습 목표**:
> - **Kinesthetic Teaching (KT)** — 사람이 직접 로봇을 끌어 가르치기 → 궤적 캡처 → 재생.
> - **투명 모드 (Ex.31 DOB)** → 교시 모드 → 재생 모드 — 3단계 워크플로우.
> - 16 KB 정적 버퍼 (2000 샘플 × 2 ms) — **AI 학습 데이터 수집의 원시 (primitive)**.
>
> ⏱️ 권장 시간: 55분 | 🔧 난이도: ⭐⭐⭐
> 🧰 사전 예제: [Ex.31 DOB](../31_Friction_Comp_DOB/) **(필수)** + [Ex.10b MSC Custom Struct](../10b_MSC_Custom_Struct/) | 📚 관련 docs: [H10 Control](../../docs/api-reference/02-h10-control-n-data.md)

> ⚠️ **재생 안전** — 처음엔 `KP_REPLAY = 0.5 Nm/deg` (낮은 게인) 부터. BTN 3 은 언제든 비상 정지.
...
| 증상 | 원인 | 해결 |
|------|------|------|
| 교시 중에 로봇이 무거움 | 투명 모드 부정확 (Ex.21/31 미적용) | DOB 적용 후 재시도 |
| 재생 시작 시 큰 토크 점프 | 시작 위치 ≠ 교시 첫 점 | 재생 전 첫 점으로 천천히 복귀 |
| 재생 진동 | KP_REPLAY 너무 큼 또는 KD_REPLAY 작음 | KP 0.3 → 0.5 단계 + KD ↑ |
| 2000 샘플 (4 초 @ 2 ms) 부족 | 긴 동작 교시 불가 | PSRAM (8 MB) 활용 또는 다운샘플링 |
| BTN3 비상 정지 안 됨 | 재생 루프에서 BTN 폴링 누락 | 매 cycle 에서 `XM_GetButtonEvent` 체크 |
| 재생 후 시작 위치로 돌아옴 | 교시 마지막 점 ≠ 첫 점 | 닫힌 루프 궤적으로 교시 |
| 좌·우 동시 교시 안 됨 | 본 예제는 한쪽 다리만 (RH 기준) | LH 변수 분리 + 버퍼 2배 |
| 재생 후 다음 사이클이 안 됨 | 단발 (one-shot) 재생 | 루프 끝에 인덱스 0 으로 리셋 |
