 ...
 *   1. XM_SetUsbStreamSource로 phai-studio에 실시간 스트리밍
...
// 더 큰 버퍼 필요 시: XM10 User Workspace(RAM_D1, 200KB) 활용 → XM_GetUserWorkspace()
...
static void _RunRecordMode(void)
{
    ...
    if (s_rec_downcnt >= RECORD_DOWNSAMPLE) {
        ...
        if (s_teach_count < MAX_TEACH_POINTS) {
            ...
            // 전문가 데이터 캡처 완료 메시지
            // XM_SetUsbStreamSource로 phai-studio에 실시간 스트리밍 가능
