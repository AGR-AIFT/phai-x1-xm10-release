 ...
 *
 * [피드백 모드 선택]
 * USE_GAIT_CYCLE_FEEDBACK (기본): H10 gaitCycle(0~100%) 기반 → Body Data 필수
 * USE_MOTOR_ANGLE_FEEDBACK (대안): 고관절 각도 직접 사용 → Body Data 불필요
 *
 * [⚠️ Body Data 전제조건]
 * gaitCycle은 H10 CM 실시간 보행 분석(1kHz) 출력입니다.
 * XM_SendUserBodyData()로 신체 데이터를 H10에 전달해야 정확히 동작합니다.
 * - 미설정 시: gaitCycle 부정확, AFO 동기화 실패 가능
...
/* 피드백 모드 선택 (하나만 활성화) */
#define USE_GAIT_CYCLE_FEEDBACK     /* 기본: gaitCycle 기반 (Body Data 필수) */
/* #define USE_MOTOR_ANGLE_FEEDBACK */ /* 대안: 모터 각도 기반 (Body Data 불필요) */
...
{
    /* ⚠️ Body Data 전제조건 (gaitCycle 기반 피드백 사용 시 필수)
     *
     * H10 CM은 실시간 보행 분석(1kHz)으로 gaitCycle, footContact,
     * forwardVelocity 등을 추정합니다. 이 분석은 사용자 신체 데이터가
     * H10에 전달되어야 정확하게 동작합니다.
     *
     * XM_SendUserBodyData()를 반드시 호출하세요:
     *   - 실측값 우선, 불가 시 추정치/표준체형 근사값 사용 가능
     *   - 미설정 시: gaitCycle 부정확, footContact 미검출 가능
     *
...
static float _GetFeedbackSignal(void)
{
    ...
    /* gaitCycle(0~100%) → [-1, +1] 정현파로 변환
     * gaitCycle은 H10 CM 보행 분석 결과. Body Data 설정 필수. */
    float phi_gc = ((float)XM.status.h10.gaitCycle / 100.0f) * 2.0f * (float)M_PI;
    return 15.0f * sinf(phi_gc);   /* 진폭 15 deg 스케일 (AFO 입력 스케일 맞춤) */
#else
    /* USE_MOTOR_ANGLE_FEEDBACK: 우측 고관절 각도를 직접 피드백으로 사용
     * Body Data 없이도 동작 가능 */
