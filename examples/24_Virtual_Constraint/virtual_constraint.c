 ...
 *
 * [⚠️ Body Data 전제조건 — 필수]
 * s = gaitCycle/100이 이 예제의 유일한 위상 기준입니다.
 * XM_SendUserBodyData() 미설정 시 gaitCycle 부정확 → 가상 구속 무효화.
...
{
    /* ⚠️ Body Data 필수 — gaitCycle이 위상 변수 s의 유일한 소스
     * 미설정 시 가상 구속 제어 무효화 */
    _SetupBodyData();
...
static void Active_Entry(void)
{
    ...
    s_prev_gait_cycle = XM.status.h10.gaitCycle;
...
static void Active_Loop(void)
{
    ...
    uint8_t gait_cycle = XM.status.h10.gaitCycle;
