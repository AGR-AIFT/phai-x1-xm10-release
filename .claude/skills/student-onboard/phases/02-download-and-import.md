...
압축 해제 위치도 중요합니다. SDK 내부 구조 (`XM10_SDK/Rev2.0/Extension_Module/...`) 가 깊어서 Windows 경로 길이 제한 (MAX_PATH = 260자) 에 쉽게 닿습니다. 그래서 **얕고 (drive 직하 권장), 영문 짧게** 잡아야 합니다.
...
# 결과 확인 — Extension_Module 폴더 안에 README.md 가 있어야 함
Test-Path C:\dev\Extension_Module\README.md
```

✅ `True` 가 출력되면 통과.

> 💡 ZIP 내부 최상위 폴더 이름이 다를 수 있습니다 (예: `Extension_Module-Rev2.0/`). 압축 해제 후 폴더명이 다르면 `Get-ChildItem C:\dev\` 로 실제 이름 확인하거나 `Extension_Module` 로 rename 하셔도 됩니다.
...
- `Select root directory` → `Browse` → 압축 해제한 폴더 안의 `.project` 가 있는 디렉토리 선택:
  - **Rev 2.0**: `C:\dev\Extension_Module\XM10_SDK\Rev2.0\Extension_Module\`
  - **Rev 1.1**: `C:\dev\Extension_Module\XM10_SDK\Rev1.1\Extension_Module\`
...
- **CubeIDE 가 프로젝트를 못 잡음** → `XM10_SDK/Rev2.0/Extension_Module/` 안에 `.project` `.cproject` 파일이 있어야 함. 없으면 잘못된 폴더 선택했거나 압축이 깨졌을 가능성 → ZIP 재다운로드.
