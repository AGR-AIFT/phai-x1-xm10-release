...
> 🎯 핵심: 호환성 매트릭스 — **XM v2.6.0 ↔ KIT H10 v2.4.0 ↔ ContentsFiles (Release 첨부본)** (혼용 금지)

> ⚠️ **버전 혼용 금지** — 최신 XM + H10 v1.0.x 또는 그 반대 조합 시 CAN-FD 프로토콜 불일치로 통신 오류. 매트릭스 그대로 적용 필수.
...
| :---: | :---: | :---: | :--- |
| **v2.6.0** | **CM v2.4.0 / SAM10 v2.4.0 / ESP32 v2.3.0** | **Release 첨부본** | **최신 — 권장** |
| v2.3.0 ~ v2.5.1 | CM v2.3.0 / SAM10 v2.3.0 / ESP32 v2.3.0 | Release 첨부본 | Previous |
| v2.0.x ~ v2.2.2 | CM v2.3.0 / SAM10 v2.3.0 / ESP32 v2.3.0 | 2025.02 ~ 2026.04 | Previous |
| v1.0.x | 기존 출하 버전 | 기존 출하 버전 | Legacy |

> **v1.0.x 사용자:** XM FW를 업그레이드하지 않고 v1.0.x를 그대로 사용하는 경우, KIT H10 FW도 기존 출하 버전을 유지해야 합니다. 최신 XM FW와 구 버전 H10 FW를 혼용하면 프로토콜 불일치로 정상 동작하지 않습니다.
...
> KIT H10 펌웨어가 이번 릴리즈에서 **CM · SAM10 v2.4.0** 으로 갱신되었습니다 (ESP32 는 v2.3.0 그대로). 컨텐츠 파일은 내용 변경 없이 파일명에 날짜만 붙었습니다.

| 파일명 | 용도 | 크기 |
| :--- | :--- | :---: |
| `SUIT_H10_Binary_YYYYMMDD.zip` | CM · SAM10 · ESP32 펌웨어 3종 묶음 | 압축을 풀면 `.bin` 3개 |
| `SUIT_ContentsFiles_YYYYMMDD.zip` | SD카드 컨텐츠 파일 (음성, FSM, 모션맵 등) | SD카드에 압축 해제 |
...
| 6 | 기존 컨텐츠 파일 **삭제** 후 새 파일 복사 | `SUIT_ContentsFiles_*.zip` 압축 해제 내용 복사 |
| 7 | SD카드 장착 → CM 재조립 | - |
...
**Q: 최신 XM FW를 설치했는데 KIT H10 FW를 업데이트하지 않으면?**
...
> 가능합니다. 단, KIT H10 FW도 기존 출하 버전을 그대로 유지해야 합니다. v1.0.x XM FW와 최신 H10 FW를 혼용하면 안 됩니다.
