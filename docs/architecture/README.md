...
XM10 보드 내부는 여러 층으로 구성되어 있습니다. 사용자가 손대는 곳은 **맨 위 한 곳** 뿐입니다.
...
| 층 | 누가 만드나 | 사용자 작업 |
|----|-----------|---------|
| **사용자 알고리즘** (`Control_Task/control_task.c`) | 사용자 | ✅ 여기만 작성 |
...
- `Control_Setup()` — 부팅 직후 1회만 실행. 초기화 코드 (LED 상태 설정, TSM 등록 등) 를 여기에.
- `Control_Loop()` — 매 1 ms 마다 호출. 알고리즘 본체를 여기에.
...
예제를 시험하려면 해당 폴더의 `.c` 파일 내용을 `XM_Apps/Control_Task/control_task.c` 에 통째로 복사해서 빌드하면 됩니다.
...
| 증상 | 원인 | 해결 |
|------|------|------|
| `main()` 에 코드를 추가했는데 동작 안 함 | `main()` 은 내부 스케줄러를 시작하고 끝남. 사용자 코드 자리는 `control_task.c` | `XM_Apps/Control_Task/control_task.c` 만 수정 |
...
- 내 코드가 1 ms 안에 끝나는지 확인: [Ex.18 Debug Monitor](https://github.com/AGR-AIFT/phai-x1-xm10-release/tree/Develop/examples/18_Debug_Monitor/)
