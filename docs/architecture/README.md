...
XM10 보드 내부는 여러 층으로 구성되어 있습니다. 학생이 손대는 곳은 **맨 위 한 곳** 뿐이에요.
...
| 층 | 누가 만드나 | 학생 작업 |
|----|-----------|---------|
| **사용자 알고리즘** (`User_Algorithm/user_app.c`) | 학생 | ✅ 여기만 작성 |
...
자동차에 비유하면 학생은 운전자입니다. 엑셀 (`XM_SetAssistTorqueRH`), 브레이크 (`XM_SetControlMode`), 핸들 (`XM_GetButtonEvent`) 만 조작하면 됩니다. 엔진/변속기/ECU 같은 내부는 모두 XM 라이브러리가 알아서 돌아갑니다.

> **왜 이렇게 나누었나?**
> CAN 통신, USB 송신, 모터 명령 파싱 같은 일을 학생이 매번 직접 짜면 — 버그가 생기기 쉽고, 한 학생이 만든 코드를 다른 학생이 못 씁니다. 공통 부분을 라이브러리로 빼고 알고리즘만 직접 작성하도록 분리해 두면, 시스템 안정성을 보장하면서도 학생은 본질 (제어 알고리즘) 에만 집중할 수 있습니다.
...
학생이 `Debug (F11)` 로 펌웨어를 올리고 나면, 다음 흐름으로 보드가 시작됩니다.
...
CAN 수신 같은 다른 작업이 동시에 돌아가지만, 학생은 신경 쓰지 않아도 됩니다. 시스템이 알아서 처리합니다.
...
세부 프로토콜은 라이브러리가 알아서 처리합니다. 학생은 함수 호출만 하면 됩니다.
