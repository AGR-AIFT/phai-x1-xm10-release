...
> **레포를 아직 안 받은 (clone 전) 상태여도 괜찮아요.** Claude Code 만 설치하고 임의의 빈 폴더에서 켠 다음, URL 만 알려주면 AI 가 clone 부터 같이 진행합니다. 자세한 흐름은 아래 [URL 만 받은 학생 (clone 전)](#url-만-받은-학생-clone-전) 섹션.
...
Anthropic 사가 만든 AI 코딩 도구입니다. 터미널 또는 VS Code 안에서 동작하고, 한국어로 "처음 시작할게" 라고만 말해도 이 레포의 `CLAUDE.md` 와 `.claude/skills/` 를 읽고 알아서 안내를 시작합니다.
...
- **AI 가 한국어를 못 알아듣나요?** — 한국어 입력 됩니다. 다만 명령어 자체 (`claude`, `git` 등) 는 영어로 입력하세요.
- **권한 프롬프트 (Allow/Deny) 가 무섭습니다** — AI 가 요청하는 권한은 브라우저 열기 (`Start-Process`), 설치 검증 (`where`), 레포 복사 (`git clone`) 정도예요. 모두 읽기·공개 작업입니다.
