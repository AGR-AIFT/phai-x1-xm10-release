...
* 이전 가이드 [1. 하드웨어 연결](01-hardware-setup.md)과 [2. 개발 환경 구축](02-software-setup.md) 완료
...
4. `Select root directory` 항목 옆의 **`Browse...`** 버튼을 눌러 `XM10_SDK/Extension_Module` 폴더를 선택합니다.
...
`Examples` 폴더의 원하는 예제 소스파일을 `XM_Apps/User_Algorithm` 폴더에 적용합니다.

**방법 A: user_app.c에 코드 복사 (권장)**

기존 `user_app.c` 파일을 열고, 예제 코드의 내용을 복사하여 붙여넣기합니다.

**방법 B: 예제 파일로 교체**

예제 `.c` 파일을 `User_Algorithm` 폴더로 이동하고, 기존 `user_app.c`는 삭제합니다.
...
3. 하단의 `Console` 탭에 빌드 과정이 표시되며, 잠시 후 **`Build Finished. 0 errors, 0 warnings.`** 메시지가 나타나면 성공입니다.

> **빌드 오류가 발생하면?** [Troubleshooting](../troubleshooting.md) 문서를 참고하세요.
...
각 예제 폴더(`examples/`) 안에 있는 `README.md` 파일을 참고하여, 실행 중인 예제가 어떤 동작을 하는지 확인해보세요.

---

**다음 단계:** [Tutorials — 단계별 학습](../tutorials/) 으로 이동하여 XM10의 다양한 기능을 마스터하세요.
