...
예상 시간은 일반 사용자 기준이에요. "고급" 의 한 학기는 **개별 예제마다 1~2 주씩** 들여 깊이 학습한다고 가정한 거예요. 짧게 훑기만 하면 며칠로도 가능하지만, 변형 실험과 논문 출처까지 따라가면 그만큼 시간이 필요합니다.

---

## 한 학기 (16 주) 수업 진도표 예시

대학 수업으로 운영할 경우 한 학기를 분할한 참고 진도표예요. 사용자 수준과 보드 수령 시점에 따라 자유롭게 조정하세요.
...
수업 운영 팁:
- 첫 2 주는 사용자가 환경 구축에서 가장 많이 막혀요. TA / 강사가 직접 점검 가능한 시간 배정 권장.
- 8 주차 미니 프로젝트는 사용자의 동기 유지에 효과적. "Ex.05~10a 까지 익힌 걸로 무엇이든" 같은 열린 주제로.
