# 사용자 5 명 가상 시뮬레이션 — UX 검증 워크스루

47 개 예제 + docs 전면 개편 (2026-05) 결과를 검증하기 위한 가상 페르소나 시나리오입니다. 각 사용자가 처음 GitHub URL 을 받은 시점부터 따라가며, "어디서 막힐 가능성이 있는가" 를 시뮬레이션합니다.

> 이 문서는 **사용자 (또는 멘토 강사) 가 직접 따라 읽으며** "내가 이 사용자가라면 막힐까?" 를 1 인칭으로 검증하는 용도입니다. 발견되는 막힘은 해당 페이지 보강의 입력으로 들어갑니다.
...
| **E. 강사 김** | 대학 강사. XM10 으로 한 학기 수업 설계 중 | 모든 환경 보유, 사용자 30 명 가르칠 예정 | 수업 진도표 + 사용자 트러블슈팅 흐름 |
...
**🚨 발견된 막힘**: 보드만 받은 상태에서 Claude Code 부터 시작하려는 사용자는 "clone 도 안 했는데 어디서 실행하라는 거지" 에서 막힐 수 있습니다.

**보강 제안**:
- `docs/getting-started/00-claude-code-quickstart.md` 첫 문단에 "**아직 clone 안 했어도 괜찮습니다** — 임의의 빈 폴더에서 `claude` 실행 후 URL 만 알려주면, Claude Code 가 clone 부터 안내합니다" 명시
...
| clone 위치 결정 | 루트 README + 02 페이지 | "C:\dev 가 무엇입니까" | "한글 없는 짧은 경로" 안내 + 경로 예시 ✅ |
...
**🚨 발견된 막힘**: CubeIDE Import 메뉴 위치를 처음 보는 사용자는 헷갈립니다. 03-first-build 의 스크린샷 placeholder 가 실제 이미지로 채워지기 전까지는 외부 검색에 의존.
...
**🚨 발견된 막힘**: 펌웨어 업로드는 됐는데 LED 시퀀스가 어떻게 보여야 정상인지 사용자가 자기 보드를 비교할 기준이 없습니다.
...
지원은 Arduino 의 `digitalWrite(13, HIGH)` 에 익숙합니다. "STM32 도 비슷할 거야" 라고 생각하면서 진입.
...
**🟢 지원 같은 사용자에게는 현재 UX 가 잘 동작**. Arduino 경험이 디딤돌이 됨.
...
| 핵심 코드 복사 | `control_task.c` 에 그대로 붙임 | 잘됨 ✅ |
...
**🟢 서준 같은 사용자에게는 5 단계 lab manual + API Reference 가 잘 작동**. 단, **Ex.15 까지 가는 학습 시간은 1 주 이상** 으로 readme 표에서 명시한 시간보다 길어질 가능성.
...
하은은 Cursor 에 익숙해서 "Claude Code 가 뭐지" 에서 멈춥니다.
...
**🚨 발견된 막힘**: AI 코딩 도구를 처음 접하는 사용자는 Cursor 와 Claude Code 의 차이를 모를 수 있습니다. 단, 이건 사용자의 메모리 `feedback_claude_code_only.md` 에 따라 "Cursor 동기화 불필요" 가 정책이라 안내를 일부러 안 한 것일 수도.
...
**🚨 발견된 막힘**: 하은이 진짜 알고 싶은 건 "내 보드 데이터를 PyTorch 로 어떻게 가져오지" 인데, 그 파이프라인이 단편적으로 흩어져 있습니다. PythonDecoder, PhAI Studio, USB MSC 로깅 → matplotlib 변환 같은 흐름이 한 페이지에 없음.
...
강사 김은 사용자 30 명을 가르쳐야 해서 "전체 진도를 짤 수 있는가" 부터 봅니다.
...
강사 김이 사용자 질문을 받았을 때 의지할 자료:

| 사용자 질문 | 강사가 보낼 링크 | 평가 |
|-----------|------------------|------|
| "빌드가 안 됩니다" | `docs/troubleshooting.md` | ✅ |
| "Ex.07 에서 PhAI Studio 가 충돌합니다" | Ex.07 README 의 ⚠️ 흔한 실수 + `.c` 파일 헤더 경고 | ✅ |
| "Rev 1.1 인데 Ex.04 핀이 안 맞습니다" | `docs/hardware/external-gpio-rev1.1.md` | ✅ (사용자가 핀맵 자료 채워야 완성) |
| "Body Data 가 무엇입니까" | `docs/api-reference/README.md#-body-data-전제조건` | ✅ |
| "Ex.27 MRAC 어렵습니다" | 학습 경로 "한 학기" 트랙으로 안내 + 사전 예제 (Ex.14, 26) | ✅ |

**🚨 발견된 막힘**: 강사 김이 30 명 사용자를 동시에 가르치려면 "이번 주 진도 = Ex.04~06" 같은 명시적 **수업 단위 진도표** 가 있으면 좋습니다. 현재는 사용자 자기주도 트랙만 있음.

**보강 제안**: `docs/tutorials/README.md` 에 "한 학기 수업 (16 주) 진도표" 예시 추가 검토. 강사 입장에서 바로 채택 가능한 자료.

### ③ 강사 → AI 도구 활용

- `.claude/skills/example-helper/` 스킬 → 강사가 사용자 질문을 받았을 때 "이거 Claude Code 에 물어봐" 라고 위임 가능 ✅
...
2. **clone 전 사용자 안내** — `docs/getting-started/00-claude-code-quickstart.md` 에 "clone 전이어도 OK" 한 줄 추가 검토.
3. **Cursor 사용자 진입** — 정책에 따라 안내 추가 검토 (현재 의도된 미언급일 수도).

### 🟡 Medium — 사용자 첫 1 주에 영향

4. **AI 학습 데이터 파이프라인 통합 페이지** — 하은 같은 AI 관심 사용자를 위해 `docs/advanced/` 에 "Ex.10b → PythonDecoder → PyTorch" 흐름 한 페이지로.
...
본 시뮬레이션 문서 자체는 강사/멘토가 새 사용자 받았을 때 "어떤 막힘이 예상되는지" 미리 보는 체크리스트로 재사용 가능합니다.
