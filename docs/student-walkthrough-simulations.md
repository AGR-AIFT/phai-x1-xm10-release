...
41 개 예제 + docs 전면 개편 (2026-05) 결과를 검증하기 위한 가상 페르소나 시나리오예요. 각 사용자가 처음 GitHub URL 을 받은 시점부터 따라가며, "어디서 막힐 가능성이 있는가" 를 시뮬레이션합니다.

> 이 문서는 **사용자 (또는 멘토 강사) 가 직접 따라 읽으며** "내가 이 사용자가라면 막힐까?" 를 1 인칭으로 검증하는 용도예요. 발견되는 막힘은 해당 페이지 보강의 입력으로 들어갑니다.
...
**🚨 발견된 막힘**: 보드만 받은 상태에서 Claude Code 부터 시작하려는 사용자는 "clone 도 안 했는데 어디서 실행하라는 거지" 에서 막힐 수 있어요.
...
**🚨 발견된 막힘**: CubeIDE Import 메뉴 위치를 처음 보는 사용자는 헷갈려요. 03-first-build 의 스크린샷 placeholder 가 실제 이미지로 채워지기 전까지는 외부 검색에 의존.
...
**🚨 발견된 막힘**: 펌웨어 업로드는 됐는데 LED 시퀀스가 어떻게 보여야 정상인지 사용자가 자기 보드를 비교할 기준이 없어요.
...
**🚨 발견된 막힘**: AI 코딩 도구를 처음 접하는 사용자는 Cursor 와 Claude Code 의 차이를 모를 수 있어요. 단, 이건 사용자의 메모리 `feedback_claude_code_only.md` 에 따라 "Cursor 동기화 불필요" 가 정책이라 안내를 일부러 안 한 것일 수도.
...
강사 김은 사용자 30 명을 가르쳐야 해서 "전체 진도를 짤 수 있는가" 부터 본다.
...
**🚨 발견된 막힘**: 강사 김이 30 명 사용자를 동시에 가르치려면 "이번 주 진도 = Ex.04~06" 같은 명시적 **수업 단위 진도표** 가 있으면 좋아요. 현재는 사용자 자기주도 트랙만 있음.
