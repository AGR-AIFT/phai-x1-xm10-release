"""
XM10 USB MSC Binary Data Decoder — Legacy V1
=================================
⚠️ 구버전(V1) 전용 디코더입니다. 파일 헤더/CRC/푸터가 없는 옛날 로그 포맷
(파일 맨 앞에 바로 레코드가 시작되는 "헤더 없는" 형식)만 읽을 수 있습니다.

현재 FW가 만드는 로그 파일은 파일 헤더(magic/CRC/푸터 포함)를 갖고 있어
이 스크립트로는 정상적으로 읽히지 않습니다 — 대신 PythonDecoder/MSC/의
data_decoder_xm10.py 를 사용하세요 (신규/구버전 로그 포맷을 모두 자동 인식합니다).
이 스크립트는 예전 로그 파일을 다시 열어봐야 할 때만 참고용으로 남겨둔 것입니다.

USB 메모리에 저장된 바이너리 데이터를 CSV로 변환합니다.

사용법:
    python data_decoder_xm10_v1.py <session_folder>
    python data_decoder_xm10_v1.py /LOGS/BasicTest
    python data_decoder_xm10_v1.py .                  # 현재 폴더

세션 폴더 구조:
    /LOGS/BasicTest/
      ├── metadata.txt            ← System이 자동 생성 (파싱 대상)
      ├── summary.txt             ← 세션 통계 (선택적 출력)
      ├── data_000_part_000.bin   ← 바이너리 데이터 (헤더 없는 V1 raw 포맷)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python data_decoder_xm10_v1.py <session_folder>")
        print("  예: python data_decoder_xm10_v1.py /LOGS/BasicTest")
        print("  예: python data_decoder_xm10_v1.py .  (현재 폴더)")
        sys.exit(1)
