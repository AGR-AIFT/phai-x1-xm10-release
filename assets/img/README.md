...
### Tier 1 — 사용자 첫 30 분 동선 (가장 먼저)

처음 보드를 받은 사용자가 "내 보드가 작동한다" 를 시각적으로 확인하는 핵심 자리예요. 사진/GIF 가 없으면 사용자가 "이게 정상인가?" 에서 막혀요.
...
사용자가 외부 부품을 연결하기 시작하는 단계. 회로 연결도가 없으면 사용자가 "어디에 꽂아야 하지?" 에서 막혀요.
...
KIT H10 외골격을 실제 작동시키는 단계. 동영상이 있으면 사용자가 "내 코드로 로봇이 움직였다" 의 임팩트를 느껴요.
...
Stage 1~5 학습 흐름의 시각 자료. 학회 발표/논문급 자료. 우선순위는 가장 낮지만, 있으면 사용자가 "내가 만든 게 이런 수준까지 갈 수 있구나" 동기 부여.
